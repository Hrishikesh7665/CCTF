"use strict";
const addCategoryModal = document.getElementById('modal-add-category');
const categoryTitle = document.getElementById('categoryTitle');
const modalButton = document.getElementById('addCategory');
const modalTitle = document.getElementById('modal-title');
const customAttribute = document.getElementById('setCID');
...
const categoryModal = new bootstrap.Modal(addCategoryModal, {
    keyboard: false
});

function showAddCategoryModal() {
    modalButton.innerHTML = "Add New Category";
    modalTitle.innerHTML = "Add New Category";
...
document.getElementById("addNewCategory").addEventListener("submit", function(event) {
    // Prevent the default form submission
    event.preventDefault();

    const form = event.target;
    const categoryInput = document.getElementById('categoryTitle');
    ...
    if (msg != '') {
        showToast(5000, 'mdi-alert-circle', 'animate__shakeX', 'text-danger', 'Incomplete Category!!', msg + ' and try again');
        return;
    }

    if (modalButton.innerHTML === "Add New Category"){
        let extraParam = document.createElement("input");
        extraParam.type = "hidden";
        extraParam.name = "addNewCategory";
        extraParam.value = "addNewCategory";
        form.appendChild(extraParam);
        form.submit();
    }else if(modalButton.innerHTML === "Save Category Name"){
        let extraParam = document.createElement("input");
        let extraParam2 = document.createElement("input");
        extraParam.type = "hidden";
        extraParam.name = "editCategory";
        extraParam.value = "editCategory";
        extraParam2.type = "hidden";
        extraParam2.name = "c_id";
...
function editCategory(cid, value) {
    modalButton.innerHTML = "Save Category Name";
...
function deleteCategory(cid) {
    ...
    document.getElementById('captcha-form').addEventListener('submit', function(event) {
        ...
        categoryValue.type = "hidden"; // Set input type to hidden
        categoryValue.name = "category"; // Set input name to identify it when form is submitted
        categoryValue.value = cid; // Set the challenge value
